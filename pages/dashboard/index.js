...
export async function getServerSideProps() {
  //Edit Ngrok
  const res = await fetch('https://nattawut009-ji560chan-gmailcom.vercel.app/api/users');
...
export default function Component({ posts }) {
  ...
  const handleDelete = async (id) => {
    ...
    if (result.isConfirmed) {
      ...
      //Edit Ngrok
      await fetch('https://nattawut009-ji560chan-gmailcom.vercel.app/api/users?id=' + id, {
        method: 'DELETE',
      });
    ...
    return (
      <>
        ...
        <TableContainer component={Paper}>
          ...
          <br></br>
          <center><button type="button" className="btn btn-success btn-lg btn-block" ><Link className="nav-link" href="/dashboard/user/add" >Add member</Link></button></center>
